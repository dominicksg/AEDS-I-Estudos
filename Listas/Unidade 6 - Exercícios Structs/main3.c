...
int main(void)
{
    ...
    Carro[0] = tira_barraN(Carro[0]);
    Carro[1] = tira_barraN(Carro[1]);

    FILE *arquivo = fopen("arquivo.txt", "w");

    // fprintf(arquivo, "%s %s %s %d", Carro[0].marca, Carro[0].modelo, Carro[0].cor, Carro[0].ano);
    arquivo_carro(Carro[0], arquivo);
    arquivo_carro(Carro[1], arquivo);
